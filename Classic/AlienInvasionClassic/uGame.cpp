...
void __fastcall TGameForm::PauseBitmapListAnimations( bool Value )
{
  ...
  {
	TRectangle *EnemyObj = (TRectangle*)( (TGridPanelLayout::TControlItem*)( Enemies->ControlCollection->Items[I] )->Control );
...
{
  TRectangle *R = GetPoolObj( ProjPool );
  R->Width = 25;
  R->Height = 3;
...
{
  TRectangle *R = GetPoolObj( EnemyPool );
  R->Width = Enemy->Width;
...
void __fastcall TGameForm::InitEnemies( )
{
  ...
  if ( Enemies->ControlCollection->Count > 0 )
  {
	...
	{
	  EnemyObj = (TRectangle *) Enemies->ControlCollection->Items[I]->Control;
...
float __fastcall TGameForm::GetTheMostRightEnemyPosition( TStringList *EnemyList )
{
  ...
  for ( I = 0; I < EnemyList->Count; I++)
  {
	TRectangle *pObj = (TRectangle *)(EnemyList->Objects[I]);
...
{
  TMemIniFile *IniFile = new TMemIniFile( SettingsFilePath + "Settings.ini" );
...
void __fastcall TGameForm::SwapListXY( TStringList *List )
{
  ...
  if ( List->Count > 0 )
	...
	{
	  TRectangle *pObj = (TRectangle*)(List->Objects[I]);
