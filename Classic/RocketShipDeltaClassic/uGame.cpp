...
void __fastcall TGameForm::PauseBitmapListAnimations( bool Value )
{
  ...
  {
	TRectangle *EnemyObj = (TRectangle*)( EnemyList->Objects[I] );
...
{
  TRectangle *Projectile = NULL;
  TRectangle *R = (TRectangle*)( GetPoolObj( ProjPool ) );
  R->Width = 200;
  R->Height = 3;
...
{
  TRectangle *R = (TRectangle*)( GetPoolObj( PeoplePool ) );
...
{
  TRectangle *R = (TRectangle*)( GetPoolObj( PeoplePoolForMinimap ) );
...
{
  TRectangle *EnemyGraphic = NULL;
  TBitmapListAnimation *AE = NULL;
  TBitmapListAnimation *AnimateEnemy = NULL;
  TRectangle *R = (TRectangle*)( GetPoolObj( EnemyPool ) );
  R->Parent = MapLayout1;
  R->Width = 50;
  R->Height = 50;
  R->RotationAngle = 0;
...
{
  TRectangle *R = (TRectangle*)( GetPoolObj( EnemyPoolForMinimap ) );
...
void __fastcall TGameForm::UpgradeEnemy( TRectangle* R, int Speed )
{
  TBitmapListAnimation *AE = NULL;
  TBitmapListAnimation *AnimateEnemy = NULL;
  TRectangle *EnemyGraphic = (TRectangle*)( R->TagObject );
...
TRectangle* __fastcall TGameForm::SpawnCollectItem( float X, float Y, int Size )
{
  ...
  TRectangle *R = (TRectangle*)( GetPoolObj( CollectPool ) );
  R->Stroke->Kind = TBrushKind::None;
  R->Fill->Kind = TBrushKind::None;
...
{
  TBitmapListAnimation *AE = NULL;
  TRectangle *R = (TRectangle*)( GetPoolObj( ExplosionPool ) );
  R->Parent = MapLayout1;
  R->Width = 75;
  R->Height = 75;
  R->RotationAngle = Angle - 90;
...
void __fastcall TGameForm::GameLoopTimer( TObject *Sender )
{
  ...
  TRectangle *PersonObj = NULL;
  TRectangle *PersonObjMini = NULL;
  TRectangle *ExplosionObj = NULL;
  TRectangle *EnemyObj = NULL;
  TRectangle *EnemyObjMini = NULL;
...
{
  TMemIniFile *IniFile = new TMemIniFile( DataFilePath + "Settings.ini" );  // .Create
...
void __fastcall TGameForm::ExitGameMsgBoxResultProc(TObject *Sender, const TModalResult AResult)
{
	switch (AResult) {
	  case mrYes:
		{
...
{
  PlasmaFence1 = new TRectangle( NULL );
  ...
  PlasmaFence2 = new TRectangle( NULL );
...
void __fastcall TGameForm::AnimateExplosionFinish( TObject *Sender )
{
  ...
  TRectangle *rect = (TRectangle*)animation->Owner;
  rect->Visible = false;
}
...
void __fastcall TGameForm::ResetPeople( )
{
  ...
  if (PeopleList)
    if ( PeopleList->Count > 0 )
      ...
      {
		TRectangle* PersonObj = (TRectangle*)( PeopleList->Objects[I] );
